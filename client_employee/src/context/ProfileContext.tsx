        ...
        const fetchData = async () => {
            try {
                ...
                if (profile.contacts.length > 0) {
                    ...
                    const fileDataWithKeysOnly : FileData[] = [];
                    const profilePic = profile.picture;
                    if(profilePic) {
                        fileDataWithKeysOnly.push({ fileKey: profilePic.toString(), url: '', filename: '', status: '', name: 'picture' });
                    }

                    const licenseFile = profile.license.licensefile;
                    if(licenseFile) {
                        fileDataWithKeysOnly.push({ fileKey: licenseFile.toString(), url: '', filename: '', status: '', name: 'license' });

                    }

                    setFiles(fileDataWithKeysOnly);
                }
            } catch (e) {
                console.error(e);
            } finally {
